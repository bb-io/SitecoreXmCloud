...
public static class SitecoreHtmlConverter
{
    ...
    public static byte[] ToHtml(IEnumerable<FieldModel> fields, BlackbirdItemMetadata metadata)
    {
        ...
        {
            bodyNode.AppendChild(BuildFieldNode(doc, field, metadata.ContentId));
        }

        return Encoding.UTF8.GetBytes(doc.DocumentNode.OuterHtml);
    }

    public static HtmlDocument Parse(string html) => html.AsHtmlDocument();

    public static string? ExtractItemIdFromHtml(string html) => ExtractItemId(Parse(html));
    ...
    public static Dictionary<string, string> ToSitecoreFields(string html) => ExtractFields(Parse(html));
    ...
    {
        SetMeta(doc, headNode, "blackbird-item-id", metadata.ContentId, replaceExisting);
        SetMeta(doc, headNode, "blackbird-ucid", metadata.ContentId, replaceExisting);
        SetMetaIfPresent(doc, headNode, "blackbird-content-name", metadata.ContentName, replaceExisting);
