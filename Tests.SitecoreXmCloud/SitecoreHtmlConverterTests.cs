...
public class SitecoreHtmlConverterTests
{
    ...
    private static readonly BlackbirdItemMetadata SampleMetadata = new(
        ContentId: "{ITEM-GUID-1}",
        Locale: "en",
        ContentName: "Sample Item",
    ...
    public void ToHtml_IncludesRequiredBlackbirdMeta()
    {
        ...
        Assert.AreEqual(SampleMetadata.ContentId, MetaContent(doc, "blackbird-ucid"));
        Assert.AreEqual(SampleMetadata.ContentName, MetaContent(doc, "blackbird-content-name"));
        ...
        Assert.AreEqual(SampleMetadata.ContentId, MetaContent(doc, "blackbird-item-id"));
    }
    ...
    public void ToHtml_AddsDataBlackbirdKeyPerField()
    {
        ...
        foreach (var node in fieldNodes)
        {
            ...
            Assert.AreEqual($"{SampleMetadata.ContentId}-{id}", node.GetAttributeValue("data-blackbird-key", null));
        }
    }
    ...
    public void ExtractItemId_ReadsUcidFirst_FallsBackToItemId()
    {
        ...
        Assert.AreEqual("ucid-value", SitecoreHtmlConverter.ExtractItemIdFromHtml(withUcid));
        Assert.AreEqual("legacy-value", SitecoreHtmlConverter.ExtractItemIdFromHtml(legacyOnly));
    }
    ...
    public void WriteMetadata_RewritesMetaAndLangForTarget()
    {
        ...
        var target = new BlackbirdItemMetadata(
            ContentId: "{ITEM-GUID-1}",
            Locale: "fr",
            ContentName: "Article Exemple",
            AdminUrl: "https://cm.example.com/admin/fr",
            SystemRef: "https://cm.example.com",
            UpdatedBy: null);

        var parsed = SitecoreHtmlConverter.Parse(System.Text.Encoding.UTF8.GetString(sourceHtml));
        ...
        Assert.AreEqual(target.ContentName, MetaContent(doc, "blackbird-content-name"));
